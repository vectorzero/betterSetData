...
export default class BetterSetData {
  private updatedData = Object.create(null) // 保存要渲染的数据
  private setDataCbQueue = [] // setData 回调队列
  private renderedFlag = false // 本轮是否被渲染过标识
  private oriSetData // 原生setData方法
  private setDataTimeStamp = 0 // setData 开始时间戳
  private timeConsuming = 0 // setData 结束耗时
  private setDataTimes = 0 // setData 次数

  constructor(private instance, private watcher, private betterOpts) {
  ...
  // 更新数据
  private updateData(path, newVal, oldVal, options) {
    // 收集本轮待更新数据
    this.updateData[path] = newVal
    // 同步更新值
  ...
  private performUpdate() {
    ...
    const updateDataKeys = Object.keys(this.updateData)
    ...
    if (dataLen || this.setDataCbQueue.length) {
      ...
      // 执行合并后的setData
      this.oriSetData.call(this.instance, this.updateData, () => {
  ...
  // diff 数据变化
  private diff(newVal, oldVal, path, options, isSubProp = false) {
    // 值相等或引用地址相同不更新数据
    if (newVal === oldVal) {
  ...
  // 对比长度变化
  private diffLength(newValLen, oldValLen, path, newVal, isSubProp, options) {
  ...
  // 调用原生setData(不会 diff)
  private useNativeSetData(data, fn) {
    return this.oriSetData.call(this.instance, data, fn)
  }

  // 输出重复数据
  private logSameValue(path, newVal) {
